...
async function callWithFallback(purpose, payload, kind) {
  // 主选 provider
  let target;
  try {
    target = resolveCallTarget(purpose);
  } catch (e) {
...
router.post('/text', async (req, res) => {
  const { system, user, purpose = 'connection_test' } = req.body || {};
...
router.post('/vision', async (req, res) => {
  const { prompt, base64, mimeType, purpose = 'jd_ocr' } = req.body || {};
